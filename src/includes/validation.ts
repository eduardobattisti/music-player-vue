...
import {
  ...
  min_value as minVal, max_value as maxValue, confirmed, not_one_of as excluded,
    ...
    configure({
      generateMessage: (ctx: any) => {
        const messages = {
          ...
          country_excluded: 'Due to restrictions, we do not accepts users from this location.',
          passwords_mismatch: "The password don't match.",
          tos: 'You must accepts the Terms of Service.',
        };
        const message = messages[ctx.rule.name] ? messages[ctx.rule.name] : `The fiel ${ctx.field} is invalid.`;
